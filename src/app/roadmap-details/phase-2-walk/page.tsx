  ...
  return (
    <SubPageLayout backButtonHref="/roadmap-details" backButtonText="&larr; Back to Roadmap Overview">
      <article className="prose prose-slate dark:prose-invert lg:prose-xl max-w-none text-slate-300 space-y-6">
        ...
        <p>The true innovation, and therefore the primary execution risk, lies not in the individual AI components but in the ambitious goal of integrating them into a unified, end-to-end multi-agent system (MAS) orchestrated by a central MCP Server. This integrated architecture offers the potential for a profound competitive advantage by solving the critical problem of context loss between siloed design stages.</p>
        ...
        <p>This section translates the high-level strategy of the &quot;Walk&quot; phase into a concrete, actionable plan. It is designed to be used by engineering managers to structure team activities, allocate resources, and navigate the transition to an AI-driven design methodology from Q1 2027 to Q4 2028.</p>
        ...
        <ul className="list-disc pl-5 space-y-2">
          <li><strong>Q1-Q2 2027 (Architecture Agent Enhancement):</strong> Evolve the Spec Analyst and Design Space Exploration (DSE) agents. The Spec Analyst will be enhanced to not just identify ambiguities in requirements but to proactively suggest formal specifications based on historical PPA data from similar past projects stored in the MCP Server. The DSE Agent will be upgraded to use more sophisticated, AI-driven PPA estimation models, creating a direct feedback loop from the results of the now-deployed PPA Optimization Agent to improve the accuracy of its architectural trade-off analysis.</li>
          ...
          <li><strong>Q1-Q2 2028 (TDD Loop & HLS Integration):</strong> Enhance the Test-Driven Development (TDD) loop. The Testbench Generator Agent will be upgraded to not only create functional tests but also to generate initial SystemVerilog Assertions (SVA) for the AutoDV formal verification agent, creating a stronger link between RTL generation and formal verification. Concurrently, pilot a new HLS Coder Agent that uses the same TDD workflow to generate optimized Verilog/VHDL from C++/SystemC specifications, raising the level of design abstraction for suitable modules.</li>
        </ul>
        ...
        <ul className="list-disc pl-5 space-y-2">
          <li><strong>Q1 2027:</strong> Procure and deploy commercial RL-based EDA tools (e.g., Synopsys DSO.ai, Cadence Cerebrus) to establish a performance baseline.</li>
          ...
          <li><strong>Q2-Q3 2027:</strong> Integrate the commercial tools with the MCP Server via its standardized API. The PPA Agent Team will begin developing custom RL models, trained on the company&apos;s proprietary historical design data, to augment the commercial tools.</li>
          <li><strong>Q4 2027:</strong> Begin a pilot program, testing the PPA agent on a live, mainstream (but not business-critical) chip design project. The agent will run in &quot;co-pilot&quot; mode, with its suggestions reviewed and implemented by the human team.</li>
        ...
        <ul className="list-disc pl-5 space-y-2">
            <li><strong>Q1 2027:</strong> The Core AI Platform Team will begin building the AutoReview and AutoDV agents. This includes integrating key formal verification (e.g., JasperGold) and static analysis tools into the MCP server&apos;s tool abstraction layer.</li>
            <li><strong>Q2-Q4 2027:</strong> Develop the &quot;verification-in-the-loop&quot; logic within LangGraph, where the Coder Agent&apos;s output is automatically routed to the AutoReview and AutoDV agents for immediate feedback.</li>
            <li><strong>Q1 2028:</strong> Implement advanced coverage-driven test generation capabilities, where the AutoDV agent can analyze coverage reports and autonomously write new tests to close identified gaps.</li>
        ...
        <p>The &quot;Agent Academy&quot; will be a permanent, evolving educational program, not a one-time training event. It will be modeled on the successful, role-specific skilling initiatives at technology leaders like Microsoft.</p>
        <ul className="list-disc pl-5 space-y-2">
            ...
            <li><strong>Action 2 (Q3 2027): Launch Tiered & Role-Specific Learning Paths.</strong> The Academy will offer distinct certification tracks tailored to different roles, moving beyond a one-size-fits-all approach :
                <ul className="list-disc pl-5 space-y-1 mt-1">
                    <li><strong className="text-white font-semibold">Level 1: AI Essentials (All Engineers):</strong> A mandatory introductory course covering the basics of LLMs, responsible AI use, and practical prompt engineering for daily tasks, inspired by Google&apos;s &quot;AI Essentials&quot;.</li>
                    <li><strong className="text-white font-semibold">Level 2: Agent Orchestrator (Power Users/Team Leads):</strong> A hands-on track focused on advanced workflow design in LangGraph, debugging agent behavior with LangSmith, and curating agent teams for specific projects.</li>
                    <li><strong className="text-white font-semibold">Level 3: Citizen Agent Developer (Advanced):</strong> A specialized track for engineers who want to build, test, and deploy their own custom agents. This will cover using Python for EDA automation and the formal process for registering agents with the MCP Server.</li>
                </ul>
            </li>
            <li><strong>Action 3 (Ongoing from Q3 2027): Make Learning Engaging and Continuous.</strong> To drive sustained engagement, the Academy will:
                <ul className="list-disc pl-5 space-y-1 mt-1">
                    <li><strong className="text-white font-semibold">Gamify Learning:</strong> Introduce challenges, leaderboards, and &quot;hackathon&quot;-style projects within the learning paths to make skill acquisition more interactive and fun, similar to Microsoft&apos;s &quot;SkillUp AI Challenge&quot;.</li>
                    <li><strong className="text-white font-semibold">Provide a Sandbox:</strong> Offer a dedicated, low-pressure &quot;AI Garage&quot; environment where engineers can freely experiment with building agents without impacting production workflows.</li>
                    <li><strong className="text-white font-semibold">Ensure Content is Current:</strong> Implement a plan to regularly update all training content to reflect the latest agent capabilities and best practices.</li>
                </ul>
            </li>
        </ul>
        ...
        <ul className="list-disc pl-5 space-y-2">
            <li><strong>Action 1 (Q1 2028): Host Quarterly &quot;Agent Hackathons&quot;.</strong> These events will challenge teams to build custom agents that solve a specific, real-world pain point in their workflow, with winning projects receiving recognition and resources for further development.</li>
            <li><strong>Action 2 (Q2 2028): Establish a Formal &quot;Citizen Developer&quot; Program.</strong> This program, modeled on Microsoft&apos;s successful governance of its Power Platform , will:
                <ul className="list-disc pl-5 space-y-1 mt-1">
            ...
            <li><strong>Action 3 (Ongoing): Launch &quot;AI Show-and-Tell&quot; Sessions.</strong> Host regular, informal sessions where engineers can demonstrate the custom agents they&apos;ve built, share successes, and discuss challenges. This fosters peer-to-peer learning and builds collective confidence in the new tools.</li>
        </ul>
        ...
        <ul className="list-disc pl-5 space-y-2">
            <li><strong>Action 1 (Ongoing): Consistent Executive Messaging.</strong> Leadership must consistently communicate the strategic vision, emphasizing that the goal is engineer augmentation, not replacement. This narrative is crucial for alleviating fear and building trust.</li>
        ...
        <p>The introduction of AI agents will transform the roles of the company&apos;s core engineering teams, elevating their focus from manual execution to strategic supervision.</p>
          ...
          <table className="min-w-full divide-y divide-slate-700 text-sm">
            ...
            <tbody className="divide-y divide-slate-700 bg-slate-800/30">
              <tr>
                ...
                <td className="px-4 py-3 align-top">Defines strategic product goals and constraints. Guides the Spec Analyst and DSE agents, curating the architectural options they explore. Leverages auto-generated RTL skeletons for top candidates to accelerate the transition to implementation. </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">Defines high-level functional requirements and test criteria for modules. Supervises the TDD workflow for both the Verilog/VHDL Coder Agent and the new HLS Coder Agent, acting as the human-in-the-loop to approve the final, test-passing RTL. </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">Oversees the AutoReview and AutoDV agents. Focuses expertise on specifying complex properties for formal verification, analyzing systemic coverage gaps identified by AI, and acting as the final arbiter on complex bugs that the AI flags but cannot resolve. </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">Defines high-level PPA goals and constraints for the PPA Optimization Agent. Analyzes the agent&apos;s recommended solutions, using LangSmith traces to understand its &quot;reasoning,&quot; and guides its exploration of the solution space for the most challenging design blocks. </td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <ul className="list-disc pl-5 space-y-2">
            <li><strong className="text-white font-semibold">Managing Complexity:</strong> The greatest technical risk is creating a chaotic, unmanageable system. <strong className="text-purple-300">Mitigation:</strong> Strictly enforce the Supervisor-Worker architecture to maintain control. The MLOps team must implement a robust CI/CD pipeline for agents before they are deployed, using evaluation-driven development to prevent performance regressions. Monitor the Supervisor agent for performance bottlenecks as the number of worker agents increases.</li>
            <li><strong className="text-white font-semibold">Ensuring Reliability (The &quot;Black Box&quot; Problem):</strong> The probabilistic nature of AI is at odds with the deterministic needs of chip design. <strong className="text-purple-300">Mitigation:</strong> Do not blindly trust AI outputs. Enforce the &quot;gauntlet&quot; approach where all AI-generated code is subjected to a rigorous, automated battery of deterministic checks, including the TDD workflow and the AIvril formal verification framework. LangSmith observability is non-negotiable for providing the transparency needed to debug failures and build trust.</li>
        </ul>
        <div className="mt-8 mb-4 flex items-center">
          <UsersRound className="h-7 w-7 text-purple-400 mr-3 flex-shrink-0" />
        ...
        <ul className="list-disc pl-5 space-y-2">
            <li><strong className="text-white font-semibold">Overcoming Cultural Resistance:</strong> Engineers may fear being replaced or distrust AI systems they don&apos;t understand. <strong className="text-purple-300">Mitigation:</strong> Proactive and consistent communication from leadership and management is critical. Frame the initiative as augmentation, not replacement, emphasizing the new, more strategic roles. The &quot;Agent Academy&quot; and internal &quot;show-and-tell&quot; sessions for successful agent-led projects are the primary tools for building skills, confidence, and trust.</li>
            <li><strong className="text-white font-semibold">Governing Citizen Development:</strong> Empowering engineers to build custom agents can lead to a &quot;wild west&quot; of redundant, unmaintained, and insecure tools. <strong className="text-purple-300">Mitigation:</strong> Model Microsoft&apos;s successful approach with its Power Platform. Any engineer-built agent must pass through a formal validation, security, and registration process managed by the Core AI Platform team before it can be added to the central MCP Server and used in a production workflow. This balances creative freedom with systemic stability.</li>
        </ul>
        ...
        <p>The core of Project Chimera&apos;s Phase 2 is the scaled deployment of autonomous agents for Power, Performance, and Area (PPA) optimization and functional verification. A review of the current market demonstrates that this approach aligns perfectly with the industry&apos;s state-of-the-art.</p>
          ...
          <table className="min-w-full divide-y divide-slate-700 text-sm">
            ...
            <thead className="bg-slate-800/50">
              <tr>
                ...
                <th scope="col" className="px-4 py-3 text-left font-medium uppercase tracking-wider text-purple-300">Project Chimera Target </th>
            ...
            <tbody className="divide-y divide-slate-700 bg-slate-800/30">
              <tr>
                ...
                <td className="px-4 py-3 align-top">Up to 25% ; 9% (5nm GPU) </td>
                <td className="px-4 py-3 align-top">7% leakage (5nm CPU) ; 17% leakage (12nm CPU) </td>
                <td className="px-4 py-3 align-top">25% smaller circuits (NVIDIA) </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">25% frequency (7nm CPU) ; &quot;hundreds of MHz&quot; (Renesas) </td>
                <td className="px-4 py-3 align-top">14% frequency (5nm CPU) </td>
                <td className="px-4 py-3 align-top">Not Specified</td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">10% (3nm SoC) </td>
                <td className="px-4 py-3 align-top">Not Specified</td>
                <td className="px-4 py-3 align-top">25% (NVIDIA) </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">75% reduction in optimization cycle (6 months to 6 weeks) </td>
                <td className="px-4 py-3 align-top">Months to 10 days </td>
                <td className="px-4 py-3 align-top">Not Specified</td>
              </tr>
            </tbody>
          </table>
          ...
          <table className="min-w-full divide-y divide-slate-700 text-sm">
            ...
            <tbody className="divide-y divide-slate-700 bg-slate-800/30">
              <tr>
                ...
                <td className="px-4 py-3 align-top">&quot;Saves days&quot; of debug; &quot;Saves weeks&quot; of engineering time </td>
                <td className="px-4 py-3 align-top">Not Specified</td>
                <td className="px-4 py-3 align-top">65% ; 28-35% </td>
              </tr>
              <tr>
                ...
                <td className="px-4 py-3 align-top">Not Specified</td>
                <td className="px-4 py-3 align-top">Up to 30% </td>
                <td className="px-4 py-3 align-top">Not Specified</td>
              </tr>
              <tr>
                <td className="px-4 py-3 align-top"><strong className="text-white font-semibold">Coverage Improvement</strong></td>
                <td className="px-4 py-3 align-top">Not Specified</td>
                <td className="px-4 py-3 align-top">&quot;Coverage goals 50x faster&quot; </td>
                <td className="px-4 py-3 align-top">&quot;10x improvement&quot; in reducing coverage holes </td>
                <td className="px-4 py-3 align-top">17.2% average improvement </td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <p>Project Chimera Phase 2 represents a significant financial commitment, likely exceeding £30 million over its three-year duration, driven by costs for specialized talent, compute infrastructure, software licensing, and custom development. The justification for this investment lies in the transformative returns, which include a projected 30-50% reduction in design time and up to 40% improvement in power efficiency. These figures are supported by real-world ROI benchmarks. For example, improvements in PPA translate directly to financial value through lower cost of goods sold (more chips per wafer) and premium product pricing. Furthermore, reducing time-to-market by even a few weeks can be worth hundreds of millions of pounds in revenue for a high-volume product. The most powerful long-term return is the amplification of engineering talent, where a single engineer can achieve the work of a much larger team, enabling the company to pursue more innovation with greater agility.</p>
        <p>A critical financial decision for Phase 2 is the implementation of an optimized hybrid compute strategy. The research highlights the massive and, critically, bursty nature of the compute workloads required for AI-driven design optimization. Relying solely on the company&apos;s existing on-premise server racks to handle these peak loads would require significant over-provisioning, leading to high capital expenditure and inefficient use of hardware that would sit idle much of the time. The most cost-effective approach is a hybrid model: leveraging our existing server racks for steady-state, baseline workloads while using the cloud for elastic, on-demand capacity to handle bursty optimization runs. Cloud providers offer the agility to dynamically scale compute resources up for an intensive, multi-week optimization run and then scale them back down, paying only for what is used. This can lead to dramatic cost savings, with the potential to reduce compute costs by up to 90% through the use of EC2 Spot Instances. The key to unlocking this hybrid model is the robust implementation of the Zero-Trust security architecture detailed in the plan, which is precisely what is needed to securely run sensitive workloads across both on-premise and public cloud infrastructure.</p>
