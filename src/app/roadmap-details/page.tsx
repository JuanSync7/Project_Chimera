  ...
  return (
    <SubPageLayout>
      <article className="prose prose-slate dark:prose-invert lg:prose-xl max-w-none text-slate-300 space-y-6">
        ...
        <div className="text-center mt-8 mb-4"> {/* Centered H3 */}
          <h3 className="text-xl font-semibold text-white !mt-10 !mb-3">Executive Summary</h3>
        </div>
        ...
        <ul className="list-disc pl-5 space-y-1 mt-1">
            <li><strong className="text-white font-semibold">Actions:</strong> Execute the large-scale data ingestion project, populating the RAG knowledge base with all targeted design documentation, manuals, and library data.<sup>1</sup> Develop and deploy the v0.5 MCP Server, focusing on the RAG API and the tool abstraction layer for simulation tools.</li>
        ...
        <ul className="list-disc pl-5 space-y-1 mt-1">
            <li><strong className="text-white font-semibold">Structure:</strong>
                <ul className="list-circle pl-5 space-y-1 mt-1">
                    ...
                    <li><strong className="text-slate-100">AI Platform Team (3-4 Engineers):</strong> The core developers building the agentic system.<sup>1</sup></li>
        ...
        <ul className="list-disc pl-5 space-y-1 mt-1">
            ...
            <li><strong className="text-white font-semibold">Process:</strong> The pipeline will automatically run the updated agent against a <strong>&quot;golden dataset&quot;</strong> of test cases—a curated set of RTL design problems with known-good solutions.<sup>1</sup></li>
        </ul>
        ...
        <ul className="list-disc pl-5 space-y-1 mt-1">
            <li><strong className="text-white font-semibold">Debugging:</strong> LangSmith traces will be the primary tool for debugging agent failures. When the Coder Agent fails a test, engineers can inspect the trace to see the exact LLM calls, the code it generated, and the tool output that caused the failure, making the system debuggable.<sup>1</sup></li>
            <li><strong className="text-white font-semibold">Human Feedback Loop:</strong> LangSmith will be used to create annotation queues. When an agent fails, the trace can be sent to an expert engineer&apos;s queue. The engineer reviews the trace, identifies the root cause, and provides a corrected example. This feedback creates a high-quality dataset used to continuously improve the agents.<sup>1</sup></li>
        </ul>
        ...
        <ul className="list-disc pl-5 space-y-2 mt-2">
            ...
            <li><strong className="text-slate-100">Verification Engineers:</strong> Will shift from post-coding test writing to pre-coding <strong>&quot;correctness definition.&quot;</strong> They will collaborate with the AI Platform Team to guide the Testbench Generator Agent, reviewing and approving the generated tests and assertions. This effectively creates the <strong>&quot;exam&quot;</strong> that the Coder Agent must pass, directly implementing the TDD workflow.<sup>1</sup></li>
        ...
        <p><strong className="text-white font-semibold">Risk: Reliability.</strong> LLMs are probabilistic and can <strong>&quot;hallucinate&quot;</strong> or generate functionally incorrect HDL code, a catastrophic failure mode in chip design.<sup>1</sup> The unpredictability of LLMs is a primary barrier to production deployment.<sup>4</sup></p>
        <p><strong className="text-white font-semibold">Mitigation:</strong> The entire pilot project is architected around this risk. The Test-Driven Development (TDD) workflow does not blindly trust the LLM&apos;s output. The Coder Agent is constrained by a pre-defined, human-vetted suite of functional tests. Its goal is not simply to <strong>&quot;write code&quot;</strong> but to <strong>&quot;write code that passes these specific tests&quot;</strong>.<sup>1</sup> This approach grounds the probabilistic LLM in a deterministic, verifiable framework, creating a tight feedback loop that catches errors immediately.</p>
        <p><strong className="text-white font-semibold">Risk: Interpretability.</strong> The <strong>&quot;black box&quot;</strong> nature of AI hinders trust and adoption because engineers are reluctant to use tools whose reasoning they cannot understand.<sup>1</sup></p>
        <p><strong className="text-white font-semibold">Mitigation:</strong> Radical transparency will be achieved via LangSmith. Every step of the agent&apos;s <strong>&quot;thought process&quot;</strong>—every LLM call, every tool use, every intermediate result—is logged and visualized.<sup>1</sup> This provides maximum possible transparency, turning the <strong>&quot;black box&quot;</strong> into a <strong>&quot;glass box&quot;</strong> and allowing engineers to perform correlational analysis to build a heuristic understanding of the agent&apos;s behavior.</p>
        ...
        <p><strong className="text-white font-semibold">Mitigation:</strong> For Phase 1, the implementation will use the simplest, most constrained version of a multi-agent system: a strict Supervisor-Worker architecture where all communication is routed through the Supervisor.<sup>1</sup> This prioritizes control and predictability over flexibility, which is the correct trade-off for an initial, high-stakes deployment. More complex architectures, like a hierarchical <strong>&quot;supervisor of supervisors,&quot;</strong> are explicitly deferred to later phases.<sup>1</sup> Performance monitoring via LangSmith will be used to proactively identify and address system bottlenecks.<sup>1</sup></p>
        ...
        <p><strong className="text-white font-semibold">Risk: Intellectual Property Theft.</strong> The MCP Server will centralize the company&apos;s <strong>&quot;crown jewels&quot;</strong>—proprietary IP, design data, and methodologies. A breach would be an existential threat.<sup>1</sup></p>
        ...
        <ul className="list-disc pl-5 space-y-1 mt-1">
            <li><strong className="text-white font-semibold">Narrative of Augmentation:</strong> Consistent communication from leadership will frame Project Chimera as a strategy to augment, not replace, engineers by automating tedious work.<sup>1</sup></li>
            ...
            <li><strong className="text-white font-semibold">Empowerment Through Participation:</strong> The inclusion of <strong>&quot;Discipline Champions&quot;</strong> in the governance model is key to building buy-in and creating internal advocates.<sup>2</sup></li>
            <li><strong className="text-white font-semibold">Start Small, Prove Value:</strong> The <strong>&quot;Crawl&quot;</strong> phase approach is itself a change management strategy. By starting with a limited-scope pilot and delivering a tangible win, the project can build credibility and enthusiasm for the broader vision.<sup>12</sup></li>
        </ul>
        ...
        <p>The TDD pilot project is the single most important strategic choice in Phase 1, as it directly mitigates the primary risk of using generative AI in a high-stakes engineering domain: unreliability. The core problem with generative AI for code is its probabilistic nature, which can lead to functionally flawed <strong>&quot;hallucinations&quot;</strong>.<sup>1</sup> The TDD workflow inverts this process. Instead of asking the AI to <strong>&quot;write code&quot;</strong> and then testing it, correctness is first defined with a human-vetted test suite. The AI&apos;s task is then narrowed to <strong>&quot;write code that makes these tests pass&quot;</strong>.<sup>1</sup> This transforms the agent from an unpredictable creator into a constrained solver.</p>
